...
const CBTChatbot = {
  ...
  FALLBACK_GUIDES: {
    ...
    'client-monitor.html': { title: '📡 Client Monitor', who: 'Platform owner (HMG / builder)', summary: 'Register client deployments and watch their live subscription status; generate registry overrides.' },
    ...
    'license.html':    { title: '📜 License Console', who: 'Owner', summary: 'Subscription lifecycle, quick-extend, offline perpetual token. Always reachable even when the portal is locked.' },
  ...
  kb: [
    ...
    { keywords: ['exam locked', "students can't enter", 'students cannot enter', 'not open error', 'exam not found', 'locked exam', 'reopen exam', 'open exam', 'is locked', 're-open', 'reopen', 'locked out', 'cant enter', "can't enter"], reply: '**"Exam not found or not open" — what it means and the fix:**\n\nAn exam code only loads when the exam is **OPEN**. Causes and fixes:\n\n1. **Locked exam** (most common): the teacher published with "No — keep locked". The student page will actually say *"this exam is currently LOCKED by your teacher"* once the updated schema is installed. **Fix:** Teacher Hub → **Assessments → select the exam → ⋮ Actions → Open** (or the batch Open action). New exams now **open by default** on publish.\n2. **Not started yet:** if a future start time is set, students see the unlock date/time and a wait room — nothing to fix.\n3. **Closing time passed:** the exam auto-closes at its scheduled close time; re-open or extend if it should still run.\n4. **Wrong code:** 6 characters, letters+digits — resend the code/link from the Assessments page.\n\nGuardrails: sharing a locked exam now warns the teacher on WhatsApp share, copy-link AND the printed Access Sheet shows a 🔴 LOCKED banner.' },
    ...
    { keywords: ['license', 'subscription', 'expiry', 'grace', 'lifetime', 'token'], reply: '**Site License (license.html) — dual engine:**\n• **Subscription lifecycle:** lifetime/active/warning (≤30 days)/grace/expired/suspended; remote registry → database row → baked config; SHA-256 tamper evidence; quick-extend +30/+90/+365 days\n• **Offline perpetual token:** the HMAC-SHA256 certificate, verifiable with zero network\n\n**The lock screen never blocks renewal:** the License console and the Admin sign-in stay reachable when the portal is locked, so the proprietor can renew from inside at any time. And an expired platform **never goes silent** — the heartbeat system keeps the database warm so renewal is always one click away.' },
    { keywords: ['subscription expired', 'platform locked', 'renew', 'renewal', 'expired subscription', 'locked portal'], reply: '**When a subscription expires:**\n\n1. **≤30 days before:** an amber banner counts down and links the renew action.\n2. **Expiry day → grace days:** a red banner shows the final grace days left.\n3. **After grace:** the portal shows a renewal screen — BUT the platform is deliberately kept alive: the **License console and Admin sign-in stay reachable** (renew from inside: License → quick-extend +30/+90/+365 days), and the **heartbeat keeps the Supabase database warm** so the project is never paused for inactivity. Renewal restores full access instantly — no rebuild, no data loss.\n4. **From the builder\'s side:** the proprietor/HMG can extend remotely via the owner RPC or the license registry override, and watch every client\'s live status on the **📡 Client Monitor** page.\n\nIn-progress exams are never interrupted mid-paper.' },
    { keywords: ['client monitor', 'monitor clients', 'client registry', 'watch clients', 'subscription status', 'track subscriptions'], reply: '**📡 Client Monitor (client-monitor.html — owner/admin only):** the builder-side registry for platforms you have deployed:\n\n• **Register** each client deployment once: name, slug, deploy URL, and the client\'s Supabase URL + anon key (public read-only — the license row is public by design).\n• **Check live:** one click polls every client\'s actual license row and shows 🟢 active/lifetime · 🟡 warning (days left) · 🟠 grace · 🔴 expired (days over) · ⏸ suspended · ❌ unreachable (possibly paused — restore guide linked).\n• **Override / bypass:** generate the exact **license-registry JSON** snippet to extend a client, grant courtesy access, or flip a subscription to lifetime — the registry beats local state on the client platform. Copy → paste into your hosted registry file → done.\n• Every change is audit-logged server-side.' },
    { keywords: ['bypass', 'override license', 'registry override', 'courtesy access', 'extend license', 'extend subscription'], reply: '**Subscription bypass & overrides (builder tools):**\n\nThere are three sanctioned ways to keep a client running or extend them:\n\n1. **License console (on their platform):** License page → quick-extend +30/+90/+365 days — uses the owner-only `extend_site_license()` RPC and re-activates instantly. The console stays reachable even while the portal is locked.\n2. **License registry (remote, from your side):** host a small `license-registry.json` you control; the client platform checks it and **the registry always wins** over local values. The 📡 Client Monitor generates the exact snippet — set a new expiry, flip status to active, or change the model to lifetime (courtesy/permanent access).\n3. **Direct RPC:** `extend_site_license(days)` as the owner from the SQL editor.\n\nAll three are owner-controlled by design; students and teachers cannot bypass anything, and every change leaves an audit trail.' },
    ...
    /* ── HELP / NAVIGATION ── */
    { keywords: ['where is', 'navigate', 'which page', 'how do i find', 'menu', 'sitemap of pages'], reply: '**Every page:** 🏠 Home (launcher) · 📝 Take Exam · 👨‍🏫 Teacher Hub · 🧪 Multi-Subject Builder · 🤖 AI Prompts · 📖 Type Guide · 🛡️ Admin · 💾 Data & Sync · 📡 Client Monitor · 🚨 Recovery · 📦 Storage · 🩺 Health · 👥 Roles · ⚙️ Settings · 📜 License · 📊 Audit · 🏅 Verify · 🔗 Link Checker · ✅ Deployment Validator · 🗺️ Feature Guide.\n\nPublic pages are reachable directly; internal tools live behind the Teacher/Admin sign-ins. The **📘 Help Center** button on any page explains every section of the current page in detail — and you can ask me "what is this page?" anywhere.' },
  ...
  buildQuickQuestions() {
    ...
    else if (pageId === 'license.html' || pageId === 'client-monitor.html') pageSpecific = ['Subscription expired — what now?', 'How do I renew/extend?'];
