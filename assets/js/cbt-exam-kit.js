...
(function (w, d) {
  ...
  var Calc = {
    ...
    ans: 0,
    history: [],
    exprs: [],              // raw expressions, newest first (↑ recall)
    ...
    FUNCS2: {
      ...
      /* --- statistics suite (sample statistics, as exam papers expect) --- */
      median: function () {
        var a = [].slice.call(arguments).sort(function (x, y) { return x - y; }), n = a.length;
        if (!n) throw new Error('median needs values');
        return n % 2 ? a[(n - 1) / 2] : (a[n / 2 - 1] + a[n / 2]) / 2;
      },
      mode: function () {
        var m = {}, best = null, bestN = 0;
        [].forEach.call(arguments, function (v) { m[v] = (m[v] || 0) + 1; if (m[v] > bestN) { bestN = m[v]; best = v; } });
        if (best === null) throw new Error('mode needs values');
        return best;
      },
      std: function () {
        var a = [].slice.call(arguments), n = a.length;
        if (n < 2) throw new Error('std needs at least 2 values');
        var mu = a.reduce(function (t, v) { return t + v; }, 0) / n;
        return Math.sqrt(a.reduce(function (t, v) { return t + (v - mu) * (v - mu); }, 0) / (n - 1));
      },
      variance: function () {
        var a = [].slice.call(arguments), n = a.length;
        if (n < 2) throw new Error('variance needs at least 2 values');
        var mu = a.reduce(function (t, v) { return t + v; }, 0) / n;
        return a.reduce(function (t, v) { return t + (v - mu) * (v - mu); }, 0) / (n - 1);
      },
      range: function () {
        return Math.max.apply(Math, arguments) - Math.min.apply(Math, arguments);
      },
      count: function () { return arguments.length; },
      roundx: function (x, n) { var p = Math.pow(10, Number(n) || 0); return Math.round(x * p) / p; }
    },
      ...
      function parsePostfix() {
        ...
        while (peek() && (peek().t === '!' || peek().t === '%')) {
          if (peek().t === '!') { eat(); v = self._fact(v); }
            ...
            /* '%' is a POSTFIX PERCENT when no operand follows (50% = 0.5),
               and a binary MODULO when an operand follows (10 % 3 = 1) —
               the disambiguation every scientific calculator applies. */
            var nxt = toks[p + 1];
            if (!nxt || ['+', '-', '*', '/', '^', ')', ',', '!', '%'].indexOf(nxt.t) !== -1) {
              eat(); v = v / 100;
            } else break;         // left for parseTerm → modulo
          }
        }
        return v;
      }
  ...
  var ExamKit = {
    ...
    renderCalculator: function () {
      ...
      var rows = inv ? [
        ...
        ['1', '2', '3', '-', 'mod(', 'C'],
        ['0', '.', 'EE', '+', '⌫', '='],
        ['median(', 'mode(', 'std(', 'variance(', 'range(', 'roundx(']
      ] : [
        ['2nd', C.angle === 'deg' ? 'DEG' : 'RAD', 'MC', 'MR', 'M+', 'M−'],
        ['sin(', 'cos(', 'tan(', '^', '(', ')'],
        ...
        ['1', '2', '3', '-', '%', 'C'],
        ['0', '.', '+/−', '+', '⌫', '='],
        ['rand', 'randint(', 'todeg(', 'torad(', 'sum(', 'mean(']
      ];
      box.innerHTML =
        ...
        '<div class="tc-calc-actions">' +
          '<button type="button" class="tc-k" data-k="HIST-UP" title="Previous expression (↑)">↑</button>' +
          '<button type="button" class="tc-k" data-k="HIST-DN" title="Next expression (↓)">↓</button>' +
          '<button type="button" class="tc-k" data-k="MS" title="Memory store">MS</button>' +
          '<button type="button" class="tc-k tc-use" data-k="USE" title="Insert the result into the answer box you last tapped">⤵ Use result</button>' +
        '</div>' +
        ...
        '<details class="tc-calc-ref"><summary>Everything this calculator understands</summary>' +
          ...
          'gcd lcm hypot min max sum mean roundx(x,n)<br>' +
          '<b>Statistics</b> median(a,b,…) mode(…) std(…) variance(…) range(…) count(…) · ' +
          'rand randint(a,b) todeg(x) torad(x)<br>' +
          '<b>Percent</b> 50% means 0.5 · 10 % 3 means modulo — decided automatically<br>' +
          '<b>Recall &amp; Use</b> ↑ ↓ (or keyboard arrows) replay your last 20 expressions · ' +
          '<b>⤵ Use result</b> types the answer straight into the box you last tapped<br>' +
          '<b>Shortcuts</b> Alt+C calculator · Alt+K maths keyboard<br>' +
      ...
      /* thousand-grouped display for whole magnitudes, raw value otherwise */
      function fmt(v) {
        var r = Math.round(v * 1e12) / 1e12;
        if (Math.abs(r) >= 1000 && Number.isInteger(r)) return r.toLocaleString('en-US');
        return String(r);
      }
      function recall(dir) {
        if (!Calc.exprs.length) return;
        if (self._histPtr === -1) self._histPtr = 0;
        else self._histPtr = Math.max(0, Math.min(Calc.exprs.length - 1, self._histPtr + dir));
        input.value = Calc.exprs[self._histPtr];
        input.focus();
      }
      function useResult() {
        var t = self._lastField;
        if (!t || !d.body.contains(t)) {
          if (typeof w.toast === 'function') w.toast('Tap inside an answer box first, then Use result.', 'info');
          return;
        }
        var st = t.selectionStart == null ? t.value.length : t.selectionStart;
        var en = t.selectionEnd == null ? t.value.length : t.selectionEnd;
        var val = String(Math.round(Calc.ans * 1e12) / 1e12);
        t.value = t.value.slice(0, st) + val + t.value.slice(en);
        try { t.focus(); t.setSelectionRange(st + val.length, st + val.length); } catch (e) {}
      ...
      function compute() {
        ...
        try {
          ...
          Calc.exprs.unshift(src);
          Calc.exprs = Calc.exprs.slice(0, 20);
          self._histPtr = -1;
      ...
      box.querySelectorAll('[data-k]').forEach(function (b) {
        b.onclick = function () {
          ...
          if (k === 'M−') { try { Calc.memory -= Calc.evaluate(input.value || '0'); } catch (e) {} self.renderCalculator(); return; }
          if (k === 'HIST-UP') { recall(-1); return; }
          if (k === 'HIST-DN') { recall(1); return; }
          if (k === 'USE') { useResult(); return; }
    ...
    mountToolbar: function (opts) {
      ...
      /* Alt+C calculator · Alt+K maths keyboard — one-hand exam shortcuts */
      if (!this._keysBound) {
        this._keysBound = true;
        d.addEventListener('keydown', function (e) {
          if (!e.altKey || e.ctrlKey || e.metaKey || e.shiftKey) return;
          var key = (e.key || '').toLowerCase();
          if (key === 'c' && opts.calculator !== false) { e.preventDefault(); self.toggleCalculator(); }
          else if ((key === 'k' || key === 'm') && opts.mathkb !== false) { e.preventDefault(); self.toggleMathKeyboard(); }
        });
      }
      this.trackFields();
      this.injectCSS();
    ...
    injectCSS: function () {
      ...
      '.tc-calc-actions{display:grid;grid-template-columns:44px 44px 44px 1fr;gap:5px;margin-bottom:5px}' +
      '.tc-calc-actions .tc-use{font-size:.78rem;font-weight:800}' +
