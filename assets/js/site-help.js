  ...
  pages: {
    ...
    'student.html': {
      ...
      sections: [
        ...
        { name: 'Exam Screen', desc: 'Timer top-right, question navigator grid (green = answered, amber = flagged, blue = current), Previous/Next, Flag for review. In multi-subject exams, sticky subject tabs sit at the very top with live (answered/total) counters — switch papers anytime without losing answers; Next flows subject to subject and the submit dialog lists every subject\'s progress. JAMB-style 8-key shortcuts work throughout: A/B/C/D pick an option, N/→ next, P/← previous, R flag, S submit.' },
        { name: '🔊 Read Aloud & ❓ How to Answer: Two accessibility helpers sit beside the Flag button (School Connect / GOSA Portal / Adewale Classroom parity). 🔊 Read Aloud (Alt+R) speaks the current question and its options using your device\'s free voices — with a chunked queue that reads long passages smoothly, instant stop (Alt+S), math spoken in words, automatic Yorùbá/Igbo/Hausa + 20-language detection with a phonetic fallback when no native voice is installed, and a ⚙ settings panel for language, voice and speed (remembered on this device). Every question style is read naturally — matching (option pool included), ordering, categorization, assertion–reason (stems first) and case studies (passage first, then the question); it never reads the answer key and stops the moment you change question, submit, switch tabs or close the page. ❓ How to Answer opens a legend explaining every question style in plain language — reading it does not use exam time. Every structured question also shows a 💡 how-to tip right above it.', desc: 'Two accessibility helpers sit beside the Flag button (School Connect / GOSA Portal parity). 🔊 Read Aloud (Alt+R) speaks the current question and its options using your device\'s free voices — matching, ordering, categorization, assertion–reason (stems read separately) and case studies (passage first, then the question) are all read naturally; it never reads the answer key and stops the moment you change question, submit or switch tabs (Alt+S stops it too). ❓ How to Answer opens a legend explaining every question style in plain language — reading it does not use exam time. Every structured question also shows a 💡 how-to tip above it.' },
    ...
    'teacher.html': {
      ...
      sections: [
        ...
        { name: '📄 Paper Exam Export (Phase 11)', desc: 'Assessments → 📄 Paper Exam opens a print-ready document: the full question paper (school header, candidate box, instructions, lettered options or ruled answer lines, subject section headers on multi-subject papers), a confidential KEEP-SECURE answer key with explanations, and an OMR-style bubble sheet. For halls without reliable power/internet, mock exams and paper archives — the digital CBT remains the source of truth.' },
        { name: '🗺 Complete Navigation', desc: 'Every platform page is reachable from the sidebar: Main + Analytics views, Teacher Tools, Platform Pages (Student Portal, Certificate Verify, Feature Guide, Deployment Validator, Link Checker, Portals Home) and Administration consoles (Admin Panel, Data & Drive Sync, Disaster Recovery, Storage, Platform Health, Roles & Approvals, Platform Settings, Site License, Audit Log, Client Monitor). Each admin page still enforces its own sign-in/role guard when opened.' },
    ...
    'cbt-multi.html': {
      ...
      sections: [
        ...
        { name: '🔐 Teacher sign-in requirement (Phase 11 fix)', desc: 'Packages must be OWNED by a teacher account, so publishing needs a Teacher Hub session. The page shows an amber banner as soon as you arrive without one (it even tells you who you are signed in as when you are) — open the Teacher Hub, sign in, come back, publish. This used to fail confusingly for signed-in teachers before Phase 11 fixed the cross-page session wiring; an admin-panel session is deliberately never used as an exam owner.' }
      ],
    ...
    'settings.html': {
      ...
      sections: [
        { name: '📣 Announcement Banner (12K)', desc: 'Broadcast a notice to the top of every page for every visitor — info (blue) or warning (amber). Empty = off. Dismissal is per visitor browser session; editing the text re-shows it everywhere. Stored in the branding JSON — no schema change.' },
